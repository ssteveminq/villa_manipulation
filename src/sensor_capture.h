...
public:
    ForceSensorCapture() : force_data_() {
        ...
        wrist_wrench_sub_ = nh.subscribe(ft_sensor_topic, 1, &ForceSensorCapture::forceSensorCallback, this);
...
public:
    OdometryCapture() : base_location_(), base_orientation_() {
        ROS_INFO("Waiting on base odom subscriber...");

        ros::NodeHandle nh;
        std::string odom_sensor_topic = "/hsrb/odom";
        odom_sub_ = nh.subscribe(odom_sensor_topic, 1, &OdometryCapture::odometryCallback, this);
