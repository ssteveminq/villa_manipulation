...
HANDLE_TO_HAND_POS = 0.095
HANDLE_GOAL_OFFSET = 0.5
HANDLE_TO_HAND_POS_X = 0.123
...
def grasp_point_callback(msg):
        print "grasp_point_callback"
        recog_pos.header.frame_id = msg.header.frame_id
        if len(msg.poses)>0:
            recog_pos.pose=msg.poses[0]
	...
	try:
		...
		while(Is_found==False):
			...
			if Is_found:
				...
				recog_pos.header.frame_id = Response.best_grasp_pose.header.frame_id
...
def opendoor(req):
	...
	try: 
		...
		whole_body.move_to_joint_positions({'head_tilt_joint':0.12})
		#grasp_point_client()
		global recog_pos
		global is_found
		...
		while 0<1:
			try:
				listener.waitForTransform(_ORIGIN_TF,_BASE_TF, rospy.Time(), rospy.Duration(10.0))
				break
			except :
				print('Fail') 
		...
		recog_pos2 = listener.transformPose(_BASE_TF, recog_pos)		
                recog_pos2.pose.position.z= recog_pos2.pose.position.z+0.007
...
ek=math.pi/2), geometry.pose(ei=math.pi/2))
		...
		whole_body.move_end_effector_by_line((0, 0, 1), 0.051)
		# Close the gripper
		wrist_wrench.reset()
	...
	# Rotate the handle
		...
		# whole_body.move_end_effector_by_line((0, 1, 0), -0.02)

		# whole_body.move_end_effector_by_arc(geometry.pose(y=recog_pos2.pose.position.x-HANDLE_TO_HAND_POS_X/2, z=0.08, ej=math.radians(90.0)), math.radians(60.0), ref_frame_id='hand_palm_link')
		# traj = JointTrajectory()
		whole_body.move_to_joint_positions({'wrist_roll_joint':latest_positions["wrist_roll_joint"]-0.7})
		whole_body.move_end_effector_by_line((0, 0, 1), -0.09)
...
def opendoor_server():
		...
		rospy.Subscriber("hsrb/head_rgbd_sensor/depth_registered/rectified_points/handle_pose",PoseArray,grasp_point_callback)
		s = rospy.Service('open_knobdoor_service', Opendoor, opendoor)
		rospy.spin()
